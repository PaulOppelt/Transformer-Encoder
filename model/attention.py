...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.W_Q(x).view(batch_size, -1, self.nhead, self.d_k)
        k = self.W_K(x).view(batch_size, -1, self.nhead, self.d_k)
        v = self.W_V(x).view(batch_size, -1, self.nhead, self.d_k)
        return self.W_O(self.Attention(q, k, v).reshape(batch_size, -1, self.d_model))
